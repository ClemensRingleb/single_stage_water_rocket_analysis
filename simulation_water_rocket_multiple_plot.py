...
# ----- Alle Größen sind in SI-Basiseinheiten angegeben, damit nur bei der Ausgabe umgerechnet werden muss -----
...
# Pygame-Setup; Einrichtung Parameter
...
def draw_rocket(screen, rocket_color, water_color, back_color, position, scale, thickness, water_percentage, exhaust_velocity, t_elapsed):
    ...
    # (hier noch in fester Skalierung, wird später danach entsprechend skaliert)

    points = {

        # großer "Hauptkörper" der Flasche
        "t_r": pg.Vector2(20, -30),  # oben-rechts
        "b_r": pg.Vector2(20, 30),  # unten-rechts
        "t_l": pg.Vector2(-20, -30),  # oben-links
        "b_l": pg.Vector2(-20, 30),  # unten-links
    ...
    # Liste der Linien mit Start und Endpunkt
    lines_to_draw = [
        ("t_r", "b_r"), ("t_l", "b_l"),
...
while running:
    ...
    # Abfrage der einzelnen Zustände
        ...
        if event.type == KEYDOWN:
            ...
            if event.key == K_r:
                ...
                if stage == 3:
                    ...
                    #zwischenspeichern der werte für den Endplot
                    
                    t_all.append(t_list)
                    v_all.append(v_list)
                    a_all.append(a_list)
                    y_all.append(y_list)
                    p_all.append(p_abs_list)

                    fill_levels.append(V_water_0 / V_rocket * 100)
    ...
    if not pausing:
        if stage == 1:
           ...
           # permanente Berechnung der angezeigten Parameter
            V_air = V_rocket - m_water / rho_water          # Luftvolumen in der Flasche
            p_abs = p_abs_0 * (V_air_0 / V_air) ** gamma    # Adiabatengleichung

            if p_abs < p_atm:
                v_exit = 0
            else:
                # Austrittsgeschwindigkeit des wassers 
                v_exit = (2 * (p_abs - p_atm) / rho_water) ** 0.5

            # Durchflussrate Wasser
            m_water_dot = rho_water * A_nozzle * v_exit

            F_thrust = m_water_dot * v_exit     # Schubkraft

            m_total = m_rocket + m_water        # Raketenmasse

            F_G = m_total * g                   # Gewichtskraft
            F_D = 1 / 2 * rho_air * v ** 2 * C_D * A_rocket             # Luftwiderstandskraft (einbezogen um realistiuscher zu werden; Annäherung)

            a = (F_thrust - F_G - F_D) / m_total        # Beschleunigung 
...
pg.quit() # Ende der animierten Darstellung

# speichern der Daten zwischen den einzelnen Plots (Multiplot), Erstellung der Graphen
# erstellte graphen werden angezigt und im Projektordner gespeichert
...
for i in range(len(t_all)):
    plt.plot(t_all[i], a_all[i],
             linestyle=linestyles[i % len(linestyles)],
             linewidth=2,
             label='{} % Wasser'.format(round(fill_levels[i],1)))
plt.xlabel("Zeit $t$ [ s ]", fontsize = 18) 
plt.ylabel("Beschleunigung $a$ [ $\dfrac{m}{s^2}$ ]", fontsize = 18)
plt.axvline(0, color="black")
plt.axhline(0, color="black")
plt.legend(loc = "upper right", fontsize = 14)
plt.grid(linestyle='-', linewidth=1)
plt.savefig('{}_time_acceleration_plot.png'.format(V_water_0 / V_rocket * 100))
plt.show()

plt.figure(figsize=(10, 6))
for i in range(len(t_all)):
    plt.plot(t_all[i], v_all[i],
             linestyle=linestyles[i % len(linestyles)],
             linewidth=2,
             label='{} % Wasser'.format(round(fill_levels[i],1)))
plt.xlabel("Zeit $t$ [ s ]", fontsize = 18) 
plt.ylabel("Geschwindigkeit $v$ [ $\dfrac{m}{s}$ ]", fontsize = 18)
plt.axvline(0, color="black")
plt.axhline(0, color="black")
plt.legend(loc = "upper right", fontsize = 14)
plt.grid(linestyle='-', linewidth=1)
plt.savefig('{}_time_velocity_plot.png'.format(V_water_0 / V_rocket * 100))
plt.show()

plt.figure(figsize=(16, 9))
for i in range(len(t_all)):
    plt.plot(t_all[i], y_all[i],
             linestyle=linestyles[i % len(linestyles)],
             linewidth=2,
             label='{} % Wasser'.format(round(fill_levels[i],1)))
plt.xlabel("Zeit $t$ [ s ]", fontsize = 18) 
plt.ylabel("Höhe $h$ [ m ]", fontsize = 18)
plt.axvline(0, color="black")
plt.axhline(0, color="black")
plt.legend(loc = "upper right", fontsize = 16)
plt.grid(linestyle='-', linewidth=1)
plt.savefig('{}_time_height_plot.png'.format(V_water_0 / V_rocket * 100))
plt.show()


for i in range(len(t_all)):
    plt.plot(t_all[i], p_all[i], 
             linestyle=linestyles[i % len(linestyles)],
             linewidth=2, 
             label='{} % Wasser'.format(round(fill_levels[i],1)))
plt.xlabel("Zeit $t$ [ s ]", fontsize = 18) 
plt.ylabel("Druck $p$ [ bar ]", fontsize = 18)
plt.axvline(0, color="black")
plt.axhline(0, color="black")
plt.legend(loc = "upper right", fontsize = 14)
plt.grid(linestyle='-', linewidth=1)
plt.savefig('{}_time_pressure_plot.png'.format(V_water_0 / V_rocket * 100))
plt.show()

# Export der gesammelten Tabellendaten als .txt und .csv Datei

data = np.column_stack((t_list, a_list, v_list, y_list, p_abs_list))
np.savetxt("data_{}.csv".format(V_water_0 / V_rocket * 100), data, delimiter = ";", header = "Zeit; Beschleunigung; Geschwindigkeit; Höhe; Druck")
np.savetxt("data_{}.txt".format(V_water_0 / V_rocket * 100), data, header = "Zeit, Beschleunigung, Geschwindigkeit, Höhe; Druck")